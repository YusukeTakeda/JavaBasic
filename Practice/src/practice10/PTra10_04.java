...
public class PTra10_04 {
	...
	public static void main(String[] args) {
		...
		User[] users = new User[3]; //User クラスが入る箱を３つ作る。
		...
		for( int i = 0; i < users.length; i++) {
			...
			users[i] = new User(); //箱にUserクラスのインスタンスを入れてく
		...
		// ★ usersに格納されている全てのインスタンスの、フィールドuserId, userNm, mail, passwordを出力してください
			for( int i = 0; i < users.length; i++) {
				System.out.println((i+1)+"人目のID: "+users[i].userId);
				System.out.println((i+1)+"人目のName: "+users[i].userNm);
				System.out.println((i+1)+"人目のmailadress: "+users[i].mail);
				System.out.println((i+1)+"人目のpassword: "+users[i].password);
